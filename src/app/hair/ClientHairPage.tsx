...
const ClientHairPage = ({ allServices}: ClientHairPageProps) => {
  // Core states only
  ...
  const applyFilters = useCallback((services: Service[]) => {
    return services.filter(service => {
      ...
      const typeMatch = serviceTypeFilter === 'all' ||
        (serviceTypeFilter === 'hair spa' && service.category?.toLowerCase().includes('spa')) ||
        (serviceTypeFilter === 'hair treatment' && service.category?.toLowerCase().includes('treatment')) ||
        (serviceTypeFilter === 'hair coloring' && service.category?.toLowerCase().includes('color')) ||
        (serviceTypeFilter === 'hair styling' && service.category?.toLowerCase().includes('styling'));
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-white to-purple-50">
      ...
      <header className="bg-linear-to-r from-blue-600 via-purple-600 to-blue-700 text-white py-12 px-4 relative overflow-hidden">
        <div className="absolute inset-0 bg-black/10"></div>
        <div className="absolute top-4 right-4 hidden md:flex space-x-2">
          <span className="bg-blue-500 text-xs px-2 py-1 rounded-full">🔥 TRENDING</span>
          <span className="bg-purple-500 text-xs px-2 py-1 rounded-full">✨ VIRAL</span>
        </div>
        <div className="relative max-w-7xl mx-auto">
          <div className="text-center">
            <h1 className="text-3xl md:text-5xl font-bold mb-4 bg-linear-to-r from-white to-blue-100 bg-clip-text text-transparent">
              HAIR CARE & STYLING
            </h1>
            <p className="text-lg md:text-xl text-blue-100 max-w-2xl mx-auto">
              FLAWLESS STYLING, TIMELESS ELEGANCE ✨
            </p>
            <p className="text-sm md:text-lg text-blue-100 max-w-2xl mx-auto mt-2">
              Transform your hair with our specialist Hair Care experts
            </p>
      ...
      <div className="bg-linear-to-r from-rose-500 to-blue-600 text-white py-3 px-4 text-center">
        <div className="max-w-7xl mx-auto flex items-center justify-center space-x-2">
          <Zap className="animate-pulse" />
          <span className="font-semibold text-sm md:text-base">TRENDING ALERT:</span>
          <span className="text-sm md:text-base">These hair services are going viral! Book now! 🚀</span>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto py-6 md:py-8 px-4 pb-20 md:pb-8">
              ...
              >
                <div className={`h-full rounded-xl p-2 md:p-3 text-center transition-all duration-300 flex flex-col items-center justify-center ${
                  selectedCategory === category 
                    ...
                    : 'bg-white text-gray-700 shadow-md hover:shadow-lg border border-blue-100'
                }`}>
        ...
        <MotionSection className="mb-8 md:mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div 
              ...
              className="bg-linear-to-br from-blue-50 to-purple-100 p-4 md:p-6 rounded-xl cursor-pointer hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-blue-200 hover:scale-102"
            >
              <div className="text-3xl md:text-5xl mb-2 md:mb-3">💇‍♀️</div>
              <h3 className="text-lg md:text-xl font-bold text-purple-800">Hair Style Quiz</h3>
              <p className="text-gray-600 text-xs md:text-sm mt-1">Find perfect hair styles tailored for you!</p>
              <span className="mt-3 md:mt-4 bg-white text-blue-600 font-semibold py-1.5 px-3 md:py-2 md:px-4 rounded-full text-xs md:text-sm border border-blue-300">
                Start Quiz
              </span>
            </div>
            <div 
              onClick={() => setShowSkinAnalysis(true)} 
              className="bg-linear-to-br from-purple-50 to-blue-100 p-4 md:p-6 rounded-xl cursor-pointer hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-purple-200 hover:scale-102"
            >
              <div className="text-3xl md:text-5xl mb-2 md:mb-3">🔍</div>
              <h3 className="text-lg md:text-xl font-bold text-purple-800">Hair Health Analysis</h3>
              <p className="text-gray-600 text-xs md:text-sm mt-1">Get personalized hair care advice.</p>
        ...
        <MotionSection className="mb-6 md:mb-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg md:text-xl font-bold text-gray-800">
              {selectedCategory === 'All' ? 'Select Your Hair Services' : selectedCategory}
            </h2>
            <button 
              onClick={() => setShowFilters(!showFilters)}
              className="flex items-center text-blue-600 font-medium hover:text-blue-700 transition-colors text-sm md:text-base"
            >
          ...
          {showFilters && (
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-md grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6 border border-blue-100">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Price Range</label>
                <select
                  value={priceRange}
                  onChange={(e) => setPriceRange(e.target.value)}
                  className="w-full p-2 md:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-sm md:text-base"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Hair Service Type</label>
                <select
        ...
        <section className="mb-8 md:hidden">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-gray-800">Featured Hair Services</h2>
            <div className="flex space-x-1">
              <button onClick={() => scrollServices('left')} className="p-1.5 rounded-full bg-white shadow-md hover:shadow-lg transition-shadow text-blue-600 hover:bg-blue-50">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={() => scrollServices('right')} className="p-1.5 rounded-full bg-white shadow-md hover:shadow-lg transition-shadow text-blue-600 hover:bg-blue-50">
        ...
        <section className="hidden md:block mb-8">
          <h2 className="text-xl font-bold text-gray-800 mb-6">
            {selectedCategory === 'All' ? 'All Hair Services' : selectedCategory} 
          ...
          {filteredAndSortedServices.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl shadow-lg">
              ...
              <h3 className="text-xl font-medium text-gray-600 mb-2">No hair services found</h3>
        ...
        <section className="bg-linear-to-r from-blue-100 to-purple-100 rounded-xl p-4 md:p-6 mb-6 md:mb-8 text-center border-2 border-blue-200">
          <div className="inline-flex items-center bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full mb-2">
            <Zap className="mr-1" />
            FLASH SALE
          </div>
          <h3 className="text-lg md:text-xl font-bold text-gray-800">35% OFF On Keratin Treatment</h3>
          <p className="text-gray-600 text-xs md:text-sm mt-1">Transform, rejuvenate, and save!</p>
        </section>

        {/* Testimonials */}
        <section className="mb-8">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6 text-center">Hair Transformations & Stories</h2>
          <div className="flex overflow-x-auto space-x-4 md:space-x-6 pb-4 scrollbar-hide">
            <TestimonialCard name="Priya S." text="My hair has never been smoother! The keratin treatment worked wonders on my frizzy hair!" image="/images/hair/keratin_treatment.webp" />
            <TestimonialCard name="Ananya R." text="The hair spa made my damaged hair silky smooth and full of life. Highly recommend!" image="/images/hair/hair_spa.webp" />
            <TestimonialCard name="Maya T." text="Best hair coloring service in town! The color looks natural and lasted for months!" image="/images/hair/hair_coloring.webp" />
          </div>
        </section>
      </main>
