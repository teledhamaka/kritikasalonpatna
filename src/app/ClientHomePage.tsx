// kritika/src/app/ClientHomePage.tsx
"use client";
...
// Fix 1: Define MotionSection props interface
interface MotionSectionProps {
  children: React.ReactNode;
  className?: string;
}

// Simple motion for critical animations only
...
// FIXED: Correct import paths for PWA Trust Components
const TrustPWAInstallPrompt = dynamic(() => import('../components/TrustPWAInstallPrompt'), {
  ssr: false
});

const TrustSecuritySection = dynamic(() => import('../components/TrustSecuritySection'), {
  ssr: false
});
...
// FIXED: Correct import paths for Android-specific Components
const AndroidInstallGuide = dynamic(() => import('../components/AndroidInstallGuide'), {
  ssr: false
});

const AndroidInstallationSection = dynamic(() => import('../components/AndroidInstallationSection'), {
  ssr: false
});
...
const ClientHomePage = ({ allServices, trendingServices }: ClientHomePageProps) => {
  // Core states only - remove heavy states
  const [selectedCategory, setSelectedCategory] = useState<string>('All');
  const [favorites, setFavorites] = useState<Set<string>>(new Set());
  const [selectedService, setSelectedService] = useState<Service | null>(null);
  const [showServiceDetail, setShowServiceDetail] = useState(false);
  
  // Fix 2: Add activeFaq state for ServiceDetailModal
  const [activeFaq, setActiveFaq] = useState<number | null>(null);
  
  // Lazy states - only initialize when needed
  ...
  }, [selectedCategory, viralServices]);
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-pink-50 via-white to-purple-50">
      {/* NEW: PWA Trust Install Prompt */}
      <TrustPWAInstallPrompt />

      {/* NEW: Android Install Guide */}
      <AndroidInstallGuide />

      {/* Critical: Header - No heavy animations */}
      <header className="bg-linear-to-r from-pink-600 via-purple-600 to-pink-700 text-white py-12 px-4 relative overflow-hidden">
        <div className="absolute inset-0 bg-black/10"></div>
        <div className="absolute top-4 right-4 hidden md:flex space-x-2">
          <span className="bg-pink-500 text-xs px-2 py-1 rounded-full">🔥 TRENDING</span>
          <span className="bg-purple-500 text-xs px-2 py-1 rounded-full">✨ VIRAL</span>
        </div>
        <div className="relative max-w-7xl mx-auto">
          <div className="text-center">
            <h1 className="text-3xl md:text-5xl font-bold mb-4 bg-linear-to-r from-white to-pink-100 bg-clip-text text-transparent">
              KRITIKA SALON
            </h1>
            <p className="text-lg md:text-xl text-pink-100 max-w-2xl mx-auto">
              WHERE EVERY WOMAN IS A HEROINE ✨
            </p>
            <p className="text-sm md:text-lg text-pink-100 max-w-2xl mx-auto mt-2">
              Transform your definition with our specialist Cosmetologist
            </p>
      ...
      <div className="bg-linear-to-r from-rose-500 to-pink-600 text-white py-3 px-4 text-center">
        <div className="max-w-7xl mx-auto flex items-center justify-center space-x-2">
          <Zap className="animate-pulse" />
          <span className="font-semibold text-sm md:text-base">TRENDING ALERT:</span>
          <span className="text-sm md:text-base">These services are going viral! Book now! 🚀</span>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto py-6 md:py-8 px-4 pb-20 md:pb-8">
        ...
        <MotionSection className="mb-8 md:mb-12">
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 gap-3 md:gap-4">
            {serviceCategories.slice(0, 17).map((category) => (
              <div 
                key={category} 
              ...
              >
                <div className={`h-full rounded-xl p-2 md:p-3 text-center transition-all duration-300 flex flex-col items-center justify-center ${
                  selectedCategory === category 
                    ? 'bg-linear-to-br from-pink-500 to-purple-600 text-white shadow-lg' 
        ...
        <MotionSection className="mb-8 md:mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div 
              ...
              className="bg-linear-to-br from-pink-50 to-purple-100 p-4 md:p-6 rounded-xl cursor-pointer hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-pink-200 hover:scale-102"
            >
              <div className="text-3xl md:text-5xl mb-2 md:mb-3">💫</div>
              <h3 className="text-lg md:text-xl font-bold text-purple-800">Beauty Profile Quiz</h3>
              <p className="text-gray-600 text-xs md:text-sm mt-1">Find perfect services tailored for you!</p>
            ...
            <div 
              ...
              className="bg-linear-to-br from-purple-50 to-pink-100 p-4 md:p-6 rounded-xl cursor-pointer hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-purple-200 hover:scale-102"
            >
              <div className="text-3xl md:text-5xl mb-2 md:mb-3">✨</div>
              <h3 className="text-lg md:text-xl font-bold text-purple-800">AI Skin Analysis</h3>
              <p className="text-gray-600 text-xs md:text-sm mt-1">Get personalized skin care advice.</p>
              <span className="mt-3 md:mt-4 bg-white text-purple-600 font-semibold py-1.5 px-3 md:py-2 md:px-4 rounded-full text-xs md:text-sm border border-purple-300">
                Analyze Skin
              </span>
            </div>
          </div>
        ...
        <MotionSection className="mb-6 md:mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              ...
              className="flex items-center text-pink-600 font-medium hover:text-pink-700 transition-colors text-sm md:text-base"
            >
          ...
          {showFilters && (
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-md grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6 border border-pink-100">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Price Range</label>
                <select
                  value={priceRange}
                  onChange={(e) => setPriceRange(e.target.value)}
                  className="w-full p-2 md:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 transition-all text-sm md:text-base"
                >
        ...
        <section className="mb-8 md:hidden">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-gray-800">Featured Services</h2>
            <div className="flex space-x-1">
              <button onClick={() => scrollServices('left')} className="p-1.5 rounded-full bg-white shadow-md hover:shadow-lg transition-shadow text-pink-600 hover:bg-pink-50">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={() => scrollServices('right')} className="p-1.5 rounded-full bg-white shadow-md hover:shadow-lg transition-shadow text-pink-600 hover:bg-pink-50">
        ...
        <section className="bg-linear-to-r from-pink-100 to-purple-100 rounded-xl p-4 md:p-6 mb-6 md:mb-8 text-center border-2 border-pink-200">
          <div className="inline-flex items-center bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full mb-2">
            <Zap className="mr-1" />
            FLASH SALE
          </div>
          <h3 className="text-lg md:text-xl font-bold text-gray-800">30% OFF On Spa Treatments</h3>
          <p className="text-gray-600 text-xs md:text-sm mt-1">Relax, rejuvenate, and save!</p>
        </section>

        {/* Testimonials */}
        <section className="mb-8">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6 text-center">Glow-ups & Stories</h2>
          <div className="flex overflow-x-auto space-x-4 md:space-x-6 pb-4 scrollbar-hide">
            <TestimonialCard name="Priya S." text="My skin has never looked better! The diamond facial is pure magic. Feeling so confident!" image="/images/skin/hydrafacial.webp" />
            <TestimonialCard name="Ananya R." text="The bridal makeup team made me feel like an absolute princess on my big day! Flawless work." image="/images/makeup/bridal_makeup.webp" />
            <TestimonialCard name="Maya T." text="Best hair spa in town. My damaged hair is now silky smooth and full of life. Highly recommend!" image="/images/hair/hair_spa.webp" />
          </div>
        </section>

        {/* NEW: Android Installation Section */}
        <AndroidInstallationSection />

        {/* NEW: Trust and Security Section */}
