...
const ClientNailPage = ({ allServices, trendingServices }: ClientNailPageProps) => {
  ...
  const getServiceUrl = useCallback((service: Service): string => {
    ...
    const category = (service.primaryCategory || service.category || 'nail').toLowerCase();
  ...
  return (
    ...
    `}>
      ...
      `}>
        ...
        {/* ========== TRENDING NAIL SERVICES — FULL IMAGE CARD SCROLLER (matches Hair/Home) ========== */}
        {hasTrendingServices && (
          <section className="bg-gradient-to-r from-orange-50 via-pink-50 to-rose-50 rounded-2xl p-4 mb-6 border border-orange-200">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="bg-gradient-to-br from-orange-500 to-pink-500 rounded-full w-10 h-10 flex items-center justify-center animate-pulse">
              ...
              <div className="flex items-center gap-2">
                <span className="bg-white text-orange-600 text-xs font-bold px-3 py-1 rounded-full border border-orange-200">
                  {trendingCount} Trending
                </span>
                <button
                  onClick={() => router.push('/nail#all')}
                  className="hidden md:flex items-center text-orange-600 text-sm font-medium hover:underline"
                >
            ...
            >
              {trendingServices.map((service, idx) => (
                <Link
                  ...
                  className={`flex-shrink-0 bg-white rounded-xl border border-orange-100 shadow-sm hover:shadow-md transition-shadow duration-200 group transform-gpu will-change-transform ${
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      <div className="flex flex-col">
                        <span className="font-bold text-orange-600 text-base">₹{service.price}</span>
                    ...
                    <button
                      ...
                      className="w-full mt-2 py-2 text-sm bg-gradient-to-r from-orange-500 to-pink-500 text-white font-medium rounded-lg hover:from-orange-600 hover:to-pink-600 transition-colors duration-150 shadow-sm flex items-center justify-center gap-1"
                    >
              ...
              {/* View All card */}
              <div
                className={`flex-shrink-0 bg-gradient-to-br from-orange-100 to-pink-100 rounded-xl border-2 border-dashed border-orange-200 flex flex-col items-center justify-center text-center cursor-pointer hover:from-orange-200 hover:to-pink-200 transition-colors duration-200 group transform-gpu ${
                  isMobile ? 'min-w-[160px] p-4' : 'min-w-[180px] p-4'
                }`}
                onClick={() => router.push('/nail#all')}
                role="button"
                tabIndex={0}
                onKeyDown={e => e.key === 'Enter' && router.push('/nail#all')}
              >
        ...
        <section className="mb-8" aria-label="Nail consultation tools">
          ...
          <div className={`grid ${isLandscape ? 'grid-cols-2 gap-2' : 'grid-cols-2 gap-3'}`}>
            <button
              ...
              className="bg-gradient-to-br from-orange-50 to-pink-100 p-4 rounded-xl hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-orange-200 touch-target ${isLandscape ? 'p-3' : ''}"
              aria-label="Take our nail style quiz"
            >
              <div className="text-3xl mb-2">💅</div>
              <h3 className={`${isLandscape ? 'text-sm' : 'text-base'} font-bold text-orange-800`}>Nail Style Quiz</h3>
              <p className={`text-gray-600 ${isLandscape ? 'text-xs' : 'text-xs mt-1'}`}>Find your perfect nail design!</p>
              <span className={`mt-3 bg-white text-orange-600 font-semibold py-1.5 px-3 rounded-full ${isLandscape ? 'text-xs' : 'text-xs'} border border-orange-300`}>Start Quiz</span>
            </button>
            <button
              onClick={() => setShowSkinAnalysis(true)}
              className="bg-gradient-to-br from-pink-50 to-orange-100 p-4 rounded-xl hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center border-2 border-pink-200 touch-target ${isLandscape ? 'p-3' : ''}"
              aria-label="Analyze your nail health"
            >
              <div className="text-3xl mb-2">🔍</div>
              <h3 className={`${isLandscape ? 'text-sm' : 'text-base'} font-bold text-pink-800`}>Nail Health Analysis</h3>
              <p className={`text-gray-600 ${isLandscape ? 'text-xs' : 'text-xs mt-1'}`}>Get personalized nail care advice.</p>
              <span className={`mt-3 bg-white text-pink-600 font-semibold py-1.5 px-3 rounded-full ${isLandscape ? 'text-xs' : 'text-xs'} border border-pink-300`}>Analyze Nails</span>
            </button>
          </div>
        </section>
        ...
        <section className="bg-gradient-to-r from-orange-50 via-pink-50 to-rose-50 rounded-2xl p-6 mb-6 border border-orange-200">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="flex flex-col items-center">
              ...
              <div className={`${isLandscape ? 'text-[10px]' : 'text-xs'} text-gray-500`}>Nail Services</div>
            </div>
        ...
        {/* Promotional Banner */}
        <section className="bg-gradient-to-r from-orange-100 via-pink-100 to-rose-100 rounded-2xl p-6 text-center border border-orange-200 mb-6">
          <div className={`inline-flex items-center gap-1.5 bg-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-full mb-3`}>
